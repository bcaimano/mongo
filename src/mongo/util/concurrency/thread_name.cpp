...
auto& getThreadName(const boost::intrusive_ptr<ThreadContext>& context) {
    return getSconce(context.get()).ptr;
}
...
boost::intrusive_ptr<ThreadName> ThreadName::set(boost::intrusive_ptr<ThreadContext> context,
    ...
    auto& current = getThreadName(context);
    if (current && *current == *name) {
        // If we already had this name set, skip it.
        return name;
    }

    auto oldThreadName = std::exchange(current, std::move(name));
    setOSThreadName(current->toString());
