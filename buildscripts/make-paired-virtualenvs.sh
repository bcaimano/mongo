...
# * The executable directory is easier to find
checkLink() {
    if [[ ! -f "$2" ]]; then
        ln -sv "$1" "$2"
    fi
}
checkLink "$venv2_bin/python" "$venv2_bin/python2"
checkLink "$venv2_bin/pip" "$venv2_bin/pip2"
checkLink "$venv2_bin" "$venv2_dir/bin"
checkLink "$venv3_bin/python" "$venv2_bin/python3"
checkLink "$venv3_bin/pip" "$venv2_bin/pip3"
checkLink "$venv3_bin" "$venv3_dir/bin"
